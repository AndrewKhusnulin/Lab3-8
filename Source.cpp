...
private:
	void on_read(const error_code & err, size_t bytes)
	{
		if (!err)
		{
			...
			index++;
			std::cout << std::endl << "Client id = " << index << std::endl;
			std::string msg(read_buffer_, bytes);
			std::string all_clients;
			if (msg.size() > 1023)
			{
				std::cout << "[-] The message is too long. Connection interrupted!" << std::endl;
				stop();
				return;
			}
	...
	{
		if (err) return 0;
		bool found = std::find(read_buffer_, read_buffer_ + bytes, '\'' ) < read_buffer_ + bytes; //2017.11.12
...
{
	SetConsoleOutputCP(65001);
	std::cout << std::endl << "*** Lab №8: Server ***" << std::endl;
